  ...
  const renderButton = () => {
    if (user.email === course.createdBy) {
      ...
      if (currentDateTime >= courseDateTime) {
        ...
        console.log(
          "Current date/time has not yet reached the course date/time"
        );
        return (
          <button disabled={true}>
            Course will start at {course.courseDate}
          </button>
        );
      }
    } else {
      ...
      if (currentDateTime >= courseDateTime) {
        ...
        console.log("Course ID: ", courseId);
        return (
          <button onClick={() => navigate(`/meeting/${courseId}`)}>
            Join Course
          </button>
        );
      } else {
          ...
          <div className="bg-slate-100 w-full md:w-2/3 lg:w-1/2 border border-black rounded-lg p-4">
            ...
            <div className="rounded border border-black p-4 flex flex-row mb-4">
              ...
              <div className="flex flex-col justify-center">
                ...
                <p className="text-xs">
                  {course.teacherBackground ||
                    "Teacher Background Not Available"}
                </p>
              </div>
            </div>
            ...
            <h2 className="text-xs font-semibold mb-2 flex items-center pb-5">
              ...
              {course.courseDate || "Date Not Available"},{" "}
              {course.courseTime || "Time Not Available"}
            </h2>
